...
export default function Busqueda({texto,finalizarSeleccion,objetoModificacion}){
    ...
    useEffect(()=>{
        ...
        const buscarAlumnosInactivos = async ()=>{
           ...
           try{
                const {data}= await Axios.get('/api/tablasgenerales/listado/obreros')
...
function Listado({alumnos,textoBusqueda,seleccionarAlumno,anchoPaginacion}){
    ...
    useEffect(()=>{
        const vector_aux = alumnos.filter(
            item=>item.obrero.toUpperCase().includes(textoBusqueda.toUpperCase()))
    ...
    <div className="bg-wheat">
        <span className="color-63 text-small inline-block absolute right-35">{alumnosEncontrados.length} obreros encontrados</span>
        ...
        {alumnosEncontrados
           ...
           .map(item=>
            <div onClick={(e)=>{seleccionarAlumno(e,item)}} className="listado-al color-63" key={`alin-${item.id_obrero}`}>
                {/* la funcion seleccionarAlumno hay que encerrarla para que no se ejecute sola  */}
                <FontAwesomeIcon className="mr-2" icon={faUser}/>
                <span>{item.obrero}</span>
            </div>
            )
        }
    </div>
    ...
    return(
         <form onSubmit={handleSubmit}>
            ...
            <input value={textoBusqueda} 
                ...
                name="busqueda" 
                id="texto-busqueda"
                title="Para buscar ingrese nombre o apellido o ubicación del obrero"
                autoComplete="off"
                placeholder="Ingrese el nombre o apellido o ubicación del obrero" 
